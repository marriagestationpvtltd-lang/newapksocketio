...
pool.getConnection()
  .then(async conn => {
    ...
    // ── Create core chat tables if not present (idempotent) ──────────────
    // Must run BEFORE any ALTER TABLE migrations below that target these tables.
    await conn.query(`
...
process.on('uncaughtException', (err) => {
  console.error('❌ Uncaught exception (exiting):', err.message, err.stack);
  // Exit so a process manager (nodemon / PM2 / systemd) can restart cleanly.
  process.exit(1);
});
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   → Network: http://<YOUR_IP>:${PORT}  (use your machine's LAN IP for mobile)`);
});
